  ...
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND (title ILIKE '%interim%chro%' OR title ILIKE '%interim%hr%director%' OR title ILIKE '%interim%people%')
...
async function getRelatedCHROJobs() {
  try {
    const sql = createDbQuery()
    const jobs = await sql`
      SELECT id, slug, title, company_name, location, is_remote, compensation, role_category, posted_date
      FROM jobs
      WHERE is_active = true
        AND (title ILIKE '%fractional%chro%' OR title ILIKE '%fractional%hr%' OR title ILIKE '%fractional%people%' OR role_category ILIKE '%hr%')
...
export default async function InterimCHROPage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimCHROJobs(), getRelatedCHROJobs()])
    ...
    <div className="min-h-screen bg-white">
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <Image
            ...
            alt="Interim CHRO Services UK - HR leadership"
            fill
            priority
            sizes="100vw"
      ...
      {(relatedJobs as any[]).length > 0 && (
        <section className="py-8 bg-gray-50 border-t border-gray-200">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="mb-4">
              <h2 className="text-xl font-bold text-gray-900">Prefer Part-Time? Fractional CHRO Roles</h2>
