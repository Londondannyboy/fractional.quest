  ...
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND (title ILIKE '%interim%ciso%' OR title ILIKE '%interim%security%')
...
async function getRelatedCISOJobs() {
  try {
    const sql = createDbQuery()
    const jobs = await sql`
      SELECT id, slug, title, company_name, location, is_remote, compensation, role_category, posted_date
      FROM jobs
      WHERE is_active = true
        AND (title ILIKE '%fractional%ciso%' OR title ILIKE '%fractional%security%' OR role_category ILIKE '%security%')
...
export default async function InterimCISOPage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimCISOJobs(), getRelatedCISOJobs()])
    ...
    <div className="min-h-screen bg-white">
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <Image
            src="https://images.pexels.com/photos/5380642/pexels-photo-5380642.jpeg?auto=compress&cs=tinysrgb&w=1920"
            alt="Interim CISO Services UK - Security leadership"
            fill
            priority
            sizes="100vw"
      ...
      {(relatedJobs as any[]).length > 0 && (
        <section className="py-8 bg-gray-50 border-t border-gray-200">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="mb-4">
              <h2 className="text-xl font-bold text-gray-900">Prefer Part-Time? Fractional CISO Roles</h2>
            ...
            <HotJobsLines
              ...
              title="Fractional CISO Jobs"
              maxJobs={10}
              viewAllHref="/fractional-ciso-jobs"
              viewAllText="View all fractional CISO jobs"
            />
          </div>
        </section>
      )}
