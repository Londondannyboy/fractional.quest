  ...
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND role_category IN ('Marketing', 'Technology', 'HR', 'Operations')
        AND title NOT ILIKE '%interim%'
        AND (country ILIKE '%UK%' OR country ILIKE '%United Kingdom%' OR location ILIKE '%UK%' OR location ILIKE '%London%' OR location ILIKE '%Manchester%' OR location ILIKE '%Edinburgh%' OR location ILIKE '%Birmingham%' OR location ILIKE '%Bristol%' OR location ILIKE '%Leeds%' OR location ILIKE '%Glasgow%' OR location ILIKE '%England%' OR location ILIKE '%Scotland%' OR location ILIKE '%Wales%')
      ORDER BY posted_date DESC NULLS LAST
      LIMIT 15
...
export default async function PartTimeCfoJobsUkPage() {
  const [stats, companies, jobs, relatedJobs] = await Promise.all([getFinanceStats(), getFeaturedCompanies(), getFinanceJobs(), getRelatedJobs()])
        ...
        <section className="py-6 bg-white border-b border-gray-100">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <HotJobsLines
              ...
              title="Latest Part-Time CFO Jobs"
              maxJobs={12}
              viewAllHref="#jobs"
              viewAllText="See all CFO jobs"
            />
          </div>
        </section>
      )}
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Explore other part-time and fractional executive roles across marketing, technology, HR, and operations.
              </p>
            </div>
            <HotJobsLines
