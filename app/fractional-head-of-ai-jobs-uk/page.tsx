...
async function getAIStats() {
  try {
    ...
    const [totalResult, remoteResult] = await Promise.all([
      sql`SELECT COUNT(*) as count FROM jobs WHERE is_active = true AND (role_category = 'Engineering' OR title ILIKE '%AI%' OR title ILIKE '%Artificial Intelligence%' OR title ILIKE '%Machine Learning%')`,
      sql`SELECT COUNT(*) as count FROM jobs WHERE is_active = true AND (role_category = 'Engineering' OR title ILIKE '%AI%' OR title ILIKE '%Artificial Intelligence%' OR title ILIKE '%Machine Learning%') AND (is_remote = true OR workplace_type = 'Remote')`
    ])
    return {
...
async function getFeaturedCompanies() {
  try {
    ...
    const companies = await sql`
      ...
      FROM jobs
      WHERE is_active = true AND (role_category = 'Engineering' OR title ILIKE '%AI%') AND company_name IS NOT NULL
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          ...
          <FAQ skipSchema={true} items={AI_FAQS} title="" />
        </div>
      </section>
