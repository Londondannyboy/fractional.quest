  ...
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND (title ILIKE '%interim%cfo%' OR title ILIKE '%interim%finance%' OR title ILIKE '%interim%financial%')
...
async function getRelatedJobs() {
  try {
    ...
    const jobs = await sql`
      ...
      FROM jobs
      WHERE is_active = true
        AND (title ILIKE '%fractional%cfo%' OR title ILIKE '%fractional%finance%' OR role_category ILIKE '%cfo%' OR role_category ILIKE '%finance%')
...
export default async function InterimCFOPage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimCFOJobs(), getRelatedJobs()])
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <Image
            ...
            alt="Interim CFO Services UK - Financial leadership"
            fill
            priority
            sizes="100vw"
      ...
      {(relatedJobs as any[]).length > 0 && (
        <section className="py-8 bg-gray-50 border-t border-gray-200">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="mb-4">
              <h2 className="text-xl font-bold text-gray-900">Prefer Part-Time? Fractional CFO Roles</h2>
              <p className="text-sm text-gray-600">Ongoing part-time CFO positions (1-3 days/week)</p>
            </div>
            <HotJobsLines
              ...
              viewAllText="View all fractional CFO jobs"
            />
          </div>
        </section>
      )}
