...
async function getInterimMarketingDirectorJobs() {
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND (title ILIKE '%interim%marketing%director%' OR title ILIKE '%interim%head%marketing%')
...
async function getRelatedMarketingDirectorJobs() {
  try {
    const sql = createDbQuery()
    const jobs = await sql`
      SELECT id, slug, title, company_name, location, is_remote, compensation, role_category, posted_date
      FROM jobs
      WHERE is_active = true
        AND (title ILIKE '%fractional%marketing%' OR role_category ILIKE '%marketing%')
...
export default async function InterimMarketingDirectorPage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimMarketingDirectorJobs(), getRelatedMarketingDirectorJobs()])
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Live Interim Marketing Director Jobs */}
        ...
        <section className="py-8 bg-white border-b border-gray-100">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <HotJobsLines
              ...
              title="Live Interim Marketing Director Jobs"
              maxJobs={10}
      ...
      {(relatedJobs as any[]).length > 0 && (
        <section className="py-8 bg-gray-50 border-t border-gray-200">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="mb-4">
              <h2 className="text-xl font-bold text-gray-900">Prefer Part-Time? Fractional Marketing Director Roles</h2>
            ...
            <HotJobsLines
              ...
              title="Fractional Marketing Director Jobs"
              maxJobs={10}
              viewAllHref="/fractional-cmo-jobs-uk"
              viewAllText="View all fractional marketing jobs"
            />
          </div>
        </section>
      )}
