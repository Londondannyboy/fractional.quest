...
async function getRelatedExecutiveJobs() {
  try {
    const sql = createDbQuery()
    const jobs = await sql`
      SELECT id, slug, title, company_name, location, is_remote, compensation, role_category, posted_date
...
export default async function InterimExecutivePage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimExecutiveJobs(), getRelatedExecutiveJobs()])
        ...
        <section className="py-8 bg-white border-b border-gray-100">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <HotJobsLines
              ...
              title="Live Interim Executive Jobs"
              maxJobs={10}
