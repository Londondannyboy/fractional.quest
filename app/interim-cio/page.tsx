  ...
  try {
    ...
    const jobs = await sql`
      ...
      WHERE is_active = true
        AND (title ILIKE '%interim%cio%' OR title ILIKE '%interim%information%')
...
async function getRelatedCIOJobs() {
  try {
    const sql = createDbQuery()
    const jobs = await sql`
      SELECT id, slug, title, company_name, location, is_remote, compensation, role_category, posted_date
      FROM jobs
      WHERE is_active = true
        AND (title ILIKE '%fractional%cio%' OR title ILIKE '%fractional%information%' OR role_category ILIKE '%cio%' OR role_category ILIKE '%technology%')
...
export default async function InterimCIOPage() {
  const [interimJobs, relatedJobs] = await Promise.all([getInterimCIOJobs(), getRelatedCIOJobs()])
    ...
    <div className="min-h-screen bg-white">
      ...
      {(relatedJobs as any[]).length > 0 && (
        <section className="py-8 bg-gray-50 border-t border-gray-200">
          <div className="max-w-4xl mx-auto px-6 lg:px-8">
            <div className="mb-4">
              <h2 className="text-xl font-bold text-gray-900">Prefer Part-Time? Fractional CIO Roles</h2>
            ...
            <HotJobsLines
              ...
              title="Fractional CIO Jobs"
              maxJobs={10}
              viewAllHref="/fractional-cio-jobs-uk"
              viewAllText="View all fractional CIO jobs"
            />
          </div>
        </section>
      )}
